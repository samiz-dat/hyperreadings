...
StandardNode.prototype.update = async function (properties) {
  ...
  await Promise.all(del.map(v => {
    ...
    // using hr._del - avoids translation from toRDF but also potentially
    // any cascading effects that should happen when property is removed
    // eg. list items stitching
    if (Array.isArray(value)) {
      return Promise.all(value.map(val => this.hr._del(spo(this.name, v, val))))
    }
    return this.hr._del(spo(this.name, v, value))
  ...
  await Promise.all(newPredicates.map(v => {
    ...
    if (Array.isArray(value)) {
      return Promise.all(value.map(val => this.hr._put(spo(this.name, v, val))))
    }
    return this.hr._put(spo(this.name, v, value))
  }))
}
...
StandardNode.prototype.properties = async function () {
  ...
  return triples.reduce((p, node) => {
    if (p[node.predicate] === undefined) p[node.predicate] = node.object
    else if (Array.isArray(p[node.predicate])) p[node.predicate].push(node.object)
    else p[node.predicate] = [p[node.predicate], node.object]
    return p
  }, {})
}
