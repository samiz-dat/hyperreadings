  ...
  if (attr === 'co:itemContent') {
    ...
    if (prev) {
      ...
      if (next) {
        ...
        // this is a total hack to stop prev.set() triggering a cascading destroy
        // which wipes out the entire chain of the container.
        // this is because next has only one parent - which is the element getting removed
        await this.hr._del(spo(prev.name, relation, this.name))
