...
ContainerNode.prototype.contains = function () {
...
ContainerNode.prototype.iterator = function (opts) {
  var next = null
...
ContainerNode.prototype.updateList = async function (nodeIds) {
...
ContainerNode.prototype.at = async function (index, opts) {
  let count = 0
...
ContainerNode.prototype.removeNodeAt = async function (index) {
...
ContainerNode.prototype.removeNodesFrom = async function (index) {
  const node = await this.at(index, { listItems: true })
  if (!node) return
  return node.destroy()
}
