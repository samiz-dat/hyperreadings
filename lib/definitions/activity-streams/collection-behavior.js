...
CollectionBehavior.itemStream = function () {
  const stream = this.hr.graph.getStream(spo(this.name, 'ao:items'))
...
CollectionBehavior.addNewPage = async function () {
  ...
  const page = await this.hr.createNode('as:CollectionPage', { 'ao:partOf': this })
  if (last) {
...
CollectionBehavior.totalItems = makeGet('as:totalItems')
...
CollectionBehavior.addItem = async function (item) {
  if (this.type === 'ao:CollectionPage') {
