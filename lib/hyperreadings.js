...
HyperReadings.prototype.contents = function (container, fn) {
  const self = this
  return this.getFirst(container)
    .then(iterate)

  function iterate (res) {
    if (!res) return null
    const id = res['?next'] || res['?first']
    // bind functions for introspection
    res.contents = self.contents.bind(self, id)
    return Promise.resolve(fn && fn(res))
      .then(() => self.getNext(id))
      .then(iterate)
  }
}
