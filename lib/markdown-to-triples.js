...
function markdownToJsonLD (text) {
  const html = marked(text)
  return htmlToJsonLD(html)
}

function htmlToJsonLD (html) {
  blankNodeCount = 0
  const graph = {
  ...
  var stack = []
  graph['@graph'].push(walk({ '@id': 'hr:root' }, doc.childNodes[0]))
  while (stack.length) {
...
function markdownToTripleStream (md) {
  const doc = markdownToJsonLD(md)
  // console.log(doc)
  return utils.jsonLDToRDFStream(doc)
}

module.exports = {
  markdownToJsonLD,
  htmlToJsonLD,
  toStream: markdownToTripleStream
}
